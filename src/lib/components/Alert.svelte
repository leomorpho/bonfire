<script lang="ts">
	...
	const alertStyles = {
		...
		warning: 'text-yellow-800 bg-yellow-50 dark:bg-gray-700 dark:text-yellow-300',
		error: 'text-red-800 bg-red-50 dark:bg-gray-700 dark:text-red-400',
		dark: 'bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-300'
	};
	...
	<div class={`my-4  items-center rounded-lg p-4 ${alertStyles[type]} ${classname}`} role="alert">
		<div class="flex">
			...
			{#if dismissible}
				<button
					type="button"
					class="-mx-1.5 -my-1.5 ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white p-1.5 text-gray-400 hover:bg-gray-100 focus:ring-2 focus:ring-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-700"
					aria-label="Close"
					onclick={closeAlert}
				>
