<script lang="ts">
	...
	let { oninput, checked = $bindable<boolean>(false), cuttoffDate = $bindable<Date>() } = $props();
</script>
	...
	{#if checked}
		...
		>
			<p class="text-center text-sm text-gray-700 dark:text-gray-300">
				Useful if you want to make sure your RSVP list does not grow after you've made reservations
				for your event.
			</p>
		</div>
	{/if}
