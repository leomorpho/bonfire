	...
	<div class="border-t pt-4">
		{#if !hasEndTime}
			...
			<p class="text-sm text-gray-500 mt-2 text-center">
				Optional: Set when your event ends
			</p>
		{:else}
			<div class="space-y-4">
				<div class="flex items-center justify-between">
					<Label class="text-base font-medium">End Time</Label>
					<Button variant="ghost" size="sm" onclick={() => hasEndTime = false}>
						Remove
					</Button>
				</div>
				...
				<div>
					...
					<p class="text-xs text-gray-500 mt-1">
						Leave blank to use the same date as start
					</p>
				</div>
				...
				<div>
					...
					<div class="flex items-center gap-2 mt-1">
						...
						<span class="text-lg">:</span>
						<DoubleDigitsPicker bind:value={endMinute} placeholder="MM" minValue={0} maxValue={59} />
	...
	{#if data.startDate && data.startTime?.hour}
		<div class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
			...
			<p class="mt-1 text-gray-700 dark:text-gray-300">
				...
				at {data.startTime.hour}:{data.startTime.minute.padStart(2, '0')} {data.startTime.ampm}
				{#if hasEndTime && data.endTime?.hour}
					- {data.endTime.hour}:{data.endTime.minute.padStart(2, '0')} {data.endTime.ampm}
