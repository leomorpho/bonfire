<script lang="ts">
	...
	onMount(() => {
		...
		uppy = new Uppy({
			...
			.use(Compressor)
			.use(Tus, {
				endpoint: `/api/tus/files?groupId=${$page.params.id}`, // Use groupId instead of eventId
			...
			.on('upload-success', (file, response) => {
				...
				toast.success(
					'Upload successful! Your group banner is being optimized and will appear shortly.',
					{
			...
			.on('upload-error', (file, error) => {
				console.error(`❌ Upload failed for group banner`, error);
				toast.error(
