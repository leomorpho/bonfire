<script lang="ts">
	...
	import { updateGroupMemberRole, removeGroupMember } from '$lib/groups';

	let { groupId, currUserId, groupCreatorId } = $props();
		...
		const unsubscribeFromMembersQuery = client.subscribe(
			client
				...
				.Where([
					and([
						['group_id', '=', groupId],
						['user_id', '!=', groupCreatorId] // Exclude the group creator
					])
				])
				.Include('user')
	...
	async function promoteToAdmin(userId: string) {
		try {
			await updateGroupMemberRole(client, groupId, userId, 'admin', currUserId);
	...
	async function promoteToEditor(userId: string) {
		try {
			await updateGroupMemberRole(client, groupId, userId, 'editor', currUserId);
	...
	async function demoteToMember(userId: string) {
		try {
			await updateGroupMemberRole(client, groupId, userId, 'member', currUserId);
	...
	async function removeMember(userId: string) {
		try {
			await removeGroupMember(client, groupId, userId);
				...
				<div transition:slide={{ duration: 300 }} class="py-3">
					<div class="mb-4">
						...
						<ul class="ml-5 list-disc pl-5">
							<li class="rounded-md px-4 py-1 text-sm">Full group management</li>
							<li class="rounded-md px-4 py-1 text-sm">Manage members and their roles</li>
							<li class="rounded-md px-4 py-1 text-sm">Admin access to all group events</li>
							<li class="rounded-md px-4 py-1 text-sm">Edit group details</li>
						</ul>
					</div>
					...
					<div class="mb-4">
						...
						<ul class="ml-5 list-disc pl-5">
							...
							<li class="rounded-md px-4 py-1 text-sm">Edit group content</li>
					...
					<div class="mb-4">
						...
						<ul class="ml-5 list-disc pl-5">
							<li class="rounded-md px-4 py-1 text-sm">Participate in group events</li>
							<li class="rounded-md px-4 py-1 text-sm">View group information</li>
			...
			{#if currentAdmins.length === 0 && currentEditors.length === 0 && currentMembers.length === 0}
				<div class="mb-5 flex w-full justify-center">
					<div class="rounded-xl bg-white p-2 px-4 text-xs dark:bg-slate-700">
						No members yet besides the group creator
					</div>
				</div>
			{/if}
		{/if}
