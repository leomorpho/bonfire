	...
	onMount(() => {
		...
		uppy = new Uppy({
			...
			restrictions: {
				minNumberOfFiles: 1,
				maxNumberOfFiles: 1,
				maxFileSize: maxMbSize * 1024 * 1024, // Limit file size to 10MB
			...
			.use(Compressor)
			.use(Tus, {
				endpoint: `/api/tus/files?organizationId=${$page.params.id}`, // Use organizationId instead of eventId
			...
			.on('upload-success', (file, response) => {
				...
				toast.success(
					'Upload successful! Your organization banner is being optimized and will appear shortly.',
					{
						duration: 10000 // 10 seconds
					}
				);
				goto(`/org/${$page.params.id}`);
			})
			.on('upload-error', (file, error) => {
				console.error(`❌ Upload failed for organization banner`, error);
				toast.error(
			...
			.on('file-added', (file) => {
				...
				try {
					let fileMeta = {
						...
						uploadFileType: UploadFileTypes.ORGANIZATION_COVER_PHOTO,
