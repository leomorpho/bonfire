<script lang="ts">
	...
	function validateName() {
		if (name.trim().length === 0) {
			...
			nameError = 'Organization name must be at least 3 characters';
		} else if (name.trim().length > 100) {
			nameError = 'Organization name must be less than 100 characters';
		} else {
			nameError = '';
		}
	}
	...
	async function handleSubmit() {
		...
		try {
			...
			await client.insert('organizations', {
				id: orgId,
				name: name.trim(),
...
<svelte:head>
	<title>Create Organization - Bonfire</title>
