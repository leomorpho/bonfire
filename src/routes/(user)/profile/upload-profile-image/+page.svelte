	...
	onMount(() => {
		...
		uppy = new Uppy({
			...
			.on('upload-success', (file, response) => {
				...
				toast.success(
					'Upload successful! Your profile picture is being optimized and will appear shortly in the app.',
					{
			...
			.on('upload-error', (file, error) => {
				console.error(`❌ Upload failed for profile image`, error);
				toast.error(
			...
			.on('file-added', (file) => {
				...
				uppy.setFileMeta(file.id, {
					...
					uploadFileType: UploadFileTypes.PROFILE_PHOTO
				});
			});
	});
</script>
